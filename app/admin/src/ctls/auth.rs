...
use service::{cache, system_login_log, system_user};
...
async fn login_by_account(
    ...
    let cache_info = cache
        .get(captcha_cache_type.clone(), &params.key, None)
        .await?;

    let captcha_code = cache_info.value::<String>()?;
    println!("{}, {}", captcha_code, params.code);
    ...
    let user = system_user::find_user_by_username(&state.db, &params.username)
        .await?
    ...
    login_after(
        system_login_log::LoginType::Account,
        addr,
        state.clone(),
        user.id,
        user_agent,
...
/// login after action
async fn login_after<T: Into<i32>>(
    login_type: T,
    addr: SocketAddr,
    state: AppState,
    user_id: i32,
    ...
    system_user::set_last_login(&state.db, user_id, &ip_address).await?;
    system_login_log::create(
        &state.db,
        system_login_log::FormParamsForCreate {
            r#type: login_type.into(),
            user_id,
            ip_address,
...
async fn login_by_mobile(
    ...
    login_after(
        system_login_log::LoginType::Mobile,
        addr,
        state.clone(),
        user.id,
        user_agent,
...
async fn get_captcha(State(state): State<AppState>) -> Result<impl IntoResponse> {
    ...
    state
        ...
        .await
        .put(
            cache::CacheType::SystemAuthLoginCaptcha,
            &key,
